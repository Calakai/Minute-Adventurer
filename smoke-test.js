...
// === BATCH 8: LEGEND MODE TESTS ===

// Test LEGEND_MODE structure
if (!LEGEND_MODE || typeof LEGEND_MODE !== 'object') err('LEGEND_MODE missing');
else {
  if (!LEGEND_MODE.scaling || typeof LEGEND_MODE.scaling.hp !== 'number') err('LEGEND_MODE missing scaling.hp');
  if (typeof LEGEND_MODE.scaling.atk !== 'number') err('LEGEND_MODE missing scaling.atk');
  if (typeof LEGEND_MODE.scaling.gold !== 'number') err('LEGEND_MODE missing scaling.gold');
  if (typeof LEGEND_MODE.scaling.xp !== 'number') err('LEGEND_MODE missing scaling.xp');
  if (!LEGEND_MODE.modifiers || !Array.isArray(LEGEND_MODE.modifiers)) err('LEGEND_MODE missing modifiers array');
  else if (LEGEND_MODE.modifiers.length !== 3) err('LEGEND_MODE should have 3 modifiers, got ' + LEGEND_MODE.modifiers.length);
  console.log('LEGEND_MODE structure validation passed.');
}

// Test legend gear items
{
  const legendKeys = ['legend_crown','legend_blade','legend_tome','legend_cloak'];
  for (const lk of legendKeys) {
    if (!ITEMS[lk]) err('Legend gear item missing: ' + lk);
    else {
      if (ITEMS[lk].rarity !== 5) err('Legend gear "' + lk + '" should be rarity 5, got ' + ITEMS[lk].rarity);
      if (!ITEMS[lk].t) err('Legend gear "' + lk + '" missing type');
    }
  }
  console.log('Legend gear items validation passed (' + legendKeys.length + ' items).');
}

// Test enemy scaling math
{
  const elderDragonHP = ENEMIES.elder_dragon ? ENEMIES.elder_dragon.mHP : 45;
  const scaledHP = Math.ceil(elderDragonHP * LEGEND_MODE.scaling.hp);
  if (ENEMIES.elder_dragon) {
    const expected = Math.ceil(ENEMIES.elder_dragon.mHP * 1.5);
    if (scaledHP !== expected) err('Legend scaling math: expected ' + expected + ' HP, got ' + scaledHP);
    else console.log('Legend scaling math verified: elder_dragon ' + ENEMIES.elder_dragon.mHP + ' HP -> ' + scaledHP + ' HP in Legend Mode.');
  } else {
    console.log('Note: elder_dragon not found, scaling math test uses fallback.');
  }
}
