...
- All game state is stored in browser `localStorage` under keys: `activeRun`, `manualSaves`, `quickSave`, `journalMeta`, and optionally `gameSettings` (high contrast, larger text). The run object includes `gold` (integer), `adventure` (string: `'muddy_trail'`, `'iron_hollows'`, or `'ashen_waste'`), `debuffs` (array), `activeBounty` (object or null), `bountyBoard` (array or null), `activeQuests` (array), `completedQuests` (object), `questProgress` (object), `map` (generated node map or null), `mapPos` (current map position or null), `mapRevealed` (object tracking revealed regions), `restBudget` ({short, long}), `dungeonState` (active dungeon or null), `encounterHistory` (recent enemy IDs), and `deferredEffects` (array). Save data is validated on load via `GS._validateSave()` which backfills missing fields.
- The game uses `index.html` (engine + UI) and `data.js` (ENEMIES, ITEMS, NPCS, SCENE_DATA, DEATH_QUOTES, BOUNTIES, QUESTS, REGION_DATA, EXPLORATION_EVENTS, DUNGEON_DEFS, ENEMY_SYNERGIES, COMBAT_BYPASS, DUNGEON_LOOT). Game logic modules (PA, SP, CL, CALC, GS, CMB, SE, MAP) are distinct from UI modules (UI, ML, CC). Play is choice-only: no text input; use the Actions button and scene/nav buttons.
- Three adventures exist: The Muddy Trail, The Iron Hollows, and The Ashen Waste. Each uses a procedurally generated node-based region map (Slay the Spire style) via the `MAP` module. Adventures have 2-3 regions with 4-6 rows of 2-3 nodes each. Node types: combat (50%), exploration (20%), elite (10%), treasure/rest (10%), shop (5%), dungeon (5%). `SE.startAdventure(advId)` generates the map and shows it. The legacy 43 scenes in SCENE_DATA are preserved for the tavern hub. `MAP.isMapMode()` checks if the player is in map navigation mode.
- Adventure map features: 3-layer encounter pools (core, roaming, rare), map shroud (node types hidden until Sight reveals them), stat-check bypasses on ~30% of combat nodes, dungeon runs (60-70% chance per adventure, 3-5 linear nodes, commitment mechanic), bounty overlays, sawtooth difficulty pacing (max intensity jump of 3 between rows), rest budget (2 short + 1 long per adventure), region transitions with tavern visit option.
